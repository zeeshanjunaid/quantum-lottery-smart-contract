...
interface IQL {
    function processDrawChunk(uint256 _hourId, uint256 _iterations) external returns (bool);
}
...
contract QuantumLotteryTest is Test {
    ...
    // VRF Config
    uint256 subId;
    bytes32 gasLane = 0x114f3da0a805b8a67d6a7a05154f14afd91811821b38e09e03ade28a24ac2166;
    ...
    event TicketPurchased(
        uint256 indexed hourId, address indexed player, QuantumLotteryTypes.TicketType ticketType, uint256 qScoreOnEntry
    );
    ...
    function setUp() public {
        usdc = new TestUSDC();
        vrf = new VRFCoordinatorV2_5Mock(0, 0, 1e18); // Set wei per unit link to 1e18
        subId = vrf.createSubscription();
        vrf.fundSubscription(subId, 100 ether);

        vm.prank(owner);
        lottery = new QuantumLottery(address(usdc), treasury, address(vrf), subId, gasLane);
    ...
    function test_DefaultDrawsAreOpen() public view {
        ...
        assertEq(uint256(lottery.getDrawStatus(hourId)), uint256(QuantumLotteryTypes.DrawStatus.OPEN));
    }
    ...
    function test_Constructor_InvalidArgsRevert() public {
        ...
        vm.prank(owner);
        new QuantumLottery(address(usdc), address(0), address(vrf), subId, gasLane);
    ...
    function test_Revert_WhenPlayerEntersSameHourTwice() public {
        _buy(alice, QuantumLotteryTypes.TicketType.Standard);
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.PlayerAlreadyEntered.selector));
        _buy(alice, QuantumLotteryTypes.TicketType.Standard);
    }
    ...
    function test_Revert_WhenDrawIsFull() public {
        ...
        for (uint256 i = 0; i < cap; i++) {
            ...
            _buy(address(uint160(i + 1)), QuantumLotteryTypes.TicketType.Standard);
        }
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.DrawIsFull.selector));
    ...
    function test_Loser_IncrementsState() public {
        ...
        assertEq(qScore, BASE_QSCORE + BASE_BONUS, "Loser qScore should increment");
    ...
    function test_Revert_WhenRequestingForCurrentHour() public {
        uint256 hourId = block.timestamp / 3600;
        _buy(alice, QuantumLotteryTypes.TicketType.Standard);
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.DrawCannotBeClosedYet.selector));
    ...
    function test_RequestWinner_TryCatch_HandlesVrfRevert() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.VrfRequestFailed.selector));
        _requestWinner(hourId);
    ...
    function test_RequestWinner_SetsStateAndSavesRequest() public {
        ...
        assertEq(uint256(lottery.getDrawStatus(hourId)), uint256(QuantumLotteryTypes.DrawStatus.CALCULATING_WINNER));
    ...
    function test_Revert_WhenNoParticipants_requestRandomWinner() public {
        ...
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.DrawHasNoParticipants.selector));
    ...
    function test_Fulfill_SetsResolvingStatusAndTotals() public {
        ...
        assertEq(uint256(lottery.getDrawStatus(hourId)), uint256(QuantumLotteryTypes.DrawStatus.RESOLVING));
    ...
    function test_CosmicSurge_OnlyActiveForScheduledHour() public {
        ...
        QuantumLottery.Participant memory p0 = lottery.getParticipant(buyHour, 0);
        QuantumLottery.Participant memory p1 = lottery.getParticipant(buyHour, 1);
        ...
        (,,,,,,,,,,,,,,, bool cosmicActive) = lottery.draws(buyHour);
        assertTrue(cosmicActive, "Cosmic surge flag should be active for this draw");
    ...
    function test_ForceResolve_RevertWhenNotStuck() public {
        ...
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.DrawNotStuck.selector));
        lottery.forceResolveDraw(hourId);
    }

    function test_ForceResolve_RevertIfNotStuck_ForResolvingAndResolved() public {
    ...
    function test_ReservedRefunds_DecrementAndWithdrawUnclaimed() public {
        ...
        uint256 expected = lottery.STANDARD_TICKET_PRICE() + lottery.QUANTUM_TICKET_PRICE();
        assertEq(lottery.getReservedRefunds(hourId), expected);

        // One claimant claims refund
        vm.prank(a);
        lottery.claimRefund(hourId);
        assertEq(lottery.getReservedRefunds(hourId), expected - lottery.STANDARD_TICKET_PRICE());
        ...
        assertTrue(beforeWithdraw > 0, "reservedRefunds should be nonzero before withdraw");
        assertEq(afterWithdraw, 0, "reservedRefunds should be zero after withdraw");
        uint256 ownerBalAfter = usdc.balanceOf(owner);
        assertTrue(ownerBalAfter > ownerBalBefore, "Owner should receive some funds");
        ...
        assertEq(lottery.getReservedRefunds(otherHour), lottery.STANDARD_TICKET_PRICE());
    }
    ...
    function test_ClaimRefund_ErrorPaths() public {
        ...
        vm.prank(a);
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.NoRefundAvailable.selector));
        lottery.claimRefund(hourId);
    ...
    function test_Fulfill_CorrectlyDistributesAndCleansUp() public {
        ...
        assertEq(lottery.getParticipantsCount(block.timestamp / 3600), 0, "Draw struct should be deleted");
        assertEq(usdc.balanceOf(address(lottery)), 0, "Lottery balance should be zero");
        assertEq(usdc.balanceOf(treasury), (prizePot * 8) / 100, "Treasury should receive 8%");
    }
    ...
    function test_CosmicSurge_TriplesLoserBonus() public {
        ...
        assertEq(qScore, BASE_QSCORE + (BASE_BONUS * 3), "Cosmic surge should triple bonus");
    }
    ...
    function test_ChunkSize_Sensitivity() public {
        ...
        uint256[4] memory sizes = [uint256(1), uint256(10), uint256(50), uint256(100)];
    ...
    function test_fuzz_BuyTickets_randomUsers(uint256 count, uint256 seed) public {
        uint256 cap = lottery.MAX_PARTICIPANTS();
        vm.assume(count <= cap);
    ...
    function test_fuzz_WeightedRandomness_properties(uint256 n, uint256 seed, uint256 rawRand) public {
        ...
        for (uint256 i = 0; i < pc; i++) {
            QuantumLottery.Participant memory part = lottery.getParticipant(hourId, i);
    ...
    function test_fuzz_QScore_cannotOverflow(uint256 rounds) public {
        ...
        for (uint256 r = 0; r < rounds; r++) {
            ...
            QuantumLottery.Participant memory p0 = lottery.getParticipant(hourId, 0);
    ...
    function test_processDrawChunk_ReentrancySafety() public {
        ...
        VRFCoordinatorV2_5Mock localVrf = new VRFCoordinatorV2_5Mock(0, 0, 1e18);
        uint256 localSub = localVrf.createSubscription();
        ...
        QuantumLottery malLottery = new QuantumLottery(address(m), treasury, address(localVrf), localSub, gasLane);
        ...
        localVrf.fulfillRandomWordsWithOverride(req, address(malLottery), words);
        ...
        assertTrue(!firstDone, "first chunk should not finish and should pick winner");
    ...
    function test_getParticipant_IndexOutOfBounds() public {
        uint256 h = block.timestamp / 3600;
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.IndexOutOfBounds.selector));
    ...
    function test_getDrawStatus_Transitions() public {
        ...
        assertEq(uint256(lottery.getDrawStatus(normalHour)), uint256(QuantumLotteryTypes.DrawStatus.OPEN));
        uint256 req = _requestWinner(normalHour);
        // status should be CALCULATING_WINNER
        assertEq(uint256(lottery.getDrawStatus(normalHour)), uint256(QuantumLotteryTypes.DrawStatus.CALCULATING_WINNER));

        uint256[] memory words = new uint256[](1);
        words[0] = 1;
        vrf.fulfillRandomWordsWithOverride(req, address(lottery), words);
        // status should be RESOLVING
        assertEq(uint256(lottery.getDrawStatus(normalHour)), uint256(QuantumLotteryTypes.DrawStatus.RESOLVING));

        // process to completion
        bool done = false;
        while (!done) done = lottery.processDrawChunk(normalHour, 50);
        // after processing, draw.status should be RESOLVED
        assertEq(uint256(lottery.getDrawStatus(normalHour)), uint256(QuantumLotteryTypes.DrawStatus.RESOLVED));
        ...
        assertEq(uint256(lottery.getDrawStatus(forceTestHour)), uint256(QuantumLotteryTypes.DrawStatus.OPEN));
        uint256 req2 = _requestWinner(forceTestHour);
        // ensure request id maps back to the forced hour and status is CALCULATING
        assertEq(
            uint256(lottery.getDrawStatus(forceTestHour)), uint256(QuantumLotteryTypes.DrawStatus.CALCULATING_WINNER)
        );
        ...
        assertEq(uint256(lottery.getDrawStatus(forceTestHour)), uint256(QuantumLotteryTypes.DrawStatus.RESOLVED));
    }
    ...
    function test_VRF_MockIntegration() public {
        ...
        uint256[] memory words = new uint256[](1);
        words[0] = 123456; // Use non-zero random value
        vrf.fulfillRandomWordsWithOverride(req, address(lottery), words);
    ...
    function test_USDC_Mock_behaviour() public {
        ...
        vm.prank(owner);
        QuantumLottery badLottery = new QuantumLottery(address(b), treasury, address(localVrf), localSub, gasLane);
    ...
    function test_Events_TicketPurchased() public {
        ...
        emit TicketPurchased(hourId, p, QuantumLotteryTypes.TicketType.Standard, BASE_QSCORE);
        vm.prank(p);
        lottery.buyTicket(QuantumLotteryTypes.TicketType.Standard);
    }

    function test_Events_WinnerSelectionRequested_and_RandomnessFulfilled() public {
    ...
    function test_Events_WinnerPicked_matchesBalances() public {
        ...
        assertTrue(secondDone, "second chunk should finish and emit WinnerPicked");

        // verify balances (players were minted 20 USDC each in this test)
        assertEq(usdc.balanceOf(p1), 20 * ONE_USDC - lottery.STANDARD_TICKET_PRICE() + winnerAmount);
    ...
    function test_request_random_multiple_times_fail() public {
        ...
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(QuantumLotteryTypes.DrawAlreadyResolvedOrCalculating.selector));
    ...
    function _requestWinner(uint256 hourId) internal returns (uint256 requestId) {
        vm.prank(owner);
    ...
    function fulfillAndProcess(uint256 hourId, uint256 requestId, uint256[] memory words, uint256 chunkSize) internal {
        vrf.fulfillRandomWordsWithOverride(requestId, address(lottery), words);
        bool done = false;
    ...
    function _buyAndResolveDraw(address p1, address p2, uint256 randomValue) internal {
        uint256 hourId = block.timestamp / 3600;
